...
- **Калькулятор** `/raschet` + секция на главной — материал/планировка/площадь/столешница/фурнитура/подсветка/ниши, живой расчёт с разбивкой. Бесплатно. Кнопка «ИИ-визуализация · 200 ₽ — скоро» (задел).
- Статические: `/fasady` (материалы + палитра реальными цветами), `/akcii`, `/o-nas`, `/kontakty` (карта — см. открытые задачи), `/uslugi` (платные услуги + оплата), `/privacy-policy`.
...
2. **Платная ИИ-визуализация в калькуляторе (~200 ₽)** — по выбранным параметрам генерировать картинку кухни; оплата 200 ₽ через ЮKassa; без оплаты расчёт бесплатный. Задел: кнопка в `src/components/calculator/Calculator.tsx`, модель `GenerationJob`, `src/lib/video.ts`, роут `api/video/generate`. Нужен провайдер image-генерации (env-ключ) + связка с ЮKassa create/webhook.
3. **Реальная ЮKassa** — задать `YOOKASSA_SHOP_ID`/`YOOKASSA_SECRET_KEY` (sandbox), протестировать оплату до статуса `paid`, настроить webhook `https://<домен>/api/payments/yookassa/webhook` в кабинете ЮKassa.
