...
## ⚡ Актуальное состояние (обновлено 2026-09-18) — читать первым

Разделы ниже писались в июле и местами устарели. Что изменилось:

- **Удалено по просьбе заказчика (2026-09-18):** статьи/гайды (`/stati`, `lib/articles.ts`, `lib/guides.ts`),
  платные ИИ-услуги (`/uslugi`, `ServiceCard`), ИИ-визуализация/рендер (`/visualizaciya`, `api/visualization`,
  `api/video`, `lib/visualization.ts`, `lib/higgsfield.ts`, `lib/video.ts`, `lib/ai-context.ts`, `public/visualizations`).
  Старые URL редиректятся (см. `redirects()` в `next.config.js`). ИИ-чат был убран ещё в августе.
  Модели Prisma `Service` и `GenerationJob` оставлены в схеме (данные не трогаем), код их не использует.
  Всё, что ниже про ИИ-консультанта, калькулятор, Higgsfield, Groq — **неактуально**.
- **Редактор сайта в админке — `/admin/site`:** тексты и фото Hero, кнопки, часы работы, салоны (адрес/телефон/карта),
  ссылки соцсетей (MAX, VK), надписи CTA, текст футера. Хранение — модель `SiteSetting` (одна строка `key="site"`, JSON),
  миграция `20260918120000_site_settings`. Значения по умолчанию и типы — `src/lib/site-config.ts` (клиент-безопасный),
  чтение/схема zod/кеш — `src/lib/site-settings.ts` (`unstable_cache`, тег `site-settings`, revalidate 60 c;
  при сохранении `PUT /api/admin/site` делает `revalidateTag` + `revalidatePath("/", "layout")` — изменения видны сразу).
  Настройки читает `RootLayout` и передаёт в `SiteChrome → Header/Footer`.
- **Два салона:** Тимирязевская ул., 2/3 (+7 995 116 72 86) и Ленинградское шоссе, 25 (+7 995 797 40 99,
  координаты 55.82788, 37.48942). Показываются в шапке, футере, на `/kontakty` (карта OSM на каждый) и внизу главной.
- **Hero:** надписи подняты в левую среднюю зону фото (на мобильном — внизу), градиент слева.
- **Шапка:** шрифт крупнее (12–13px) при прежней высоте рядов; поверх фото — белый с тенью.
- **Проекты:** `/portfolio` — сетка одинаковых карточек 3:4 (1/2/3/4 колонки), карточка целиком влезает в экран.
  `/portfolio/[id]` — слева фото одного формата 3:4 с лайтбоксом (`ProjectPhotos`), справа «липкое» описание;
  вместо плашек Тип/Материал/Срок и кнопок — полоса «Понравился проект?» + иконки MAX/VK (`SocialLinks`) + телефон.
  Иконка соцсети выводится, только если в `/admin/site` задана ссылка.
- **Локальная разработка:** БД — контейнер `symphony-dev-db` (Postgres 16, порт **5433**; `docker start symphony-dev-db`),
  переменные — `.env.local` (в .gitignore). Dev-сервер для превью: конфигурация `symphony` (порт 3200)
  в `/Users/marat/Новая папка/.claude/launch.json`.
- **Хостинг:** судя по коду (`/opt/symphony/uploads` bind-mount), прод переехал с Render на собственный сервер —
  уточнить у владельца, как именно выкатывается `main`.
...
3. ~~**Живая ИИ-визуализация под точные параметры**~~ ✅ **КОД ГОТОВ (2026-07-08).** `src/lib/higgsfield.ts` — REST-клиент Higgsfield (`POST https://platform.higgsfield.ai/{model}` + поллинг `GET /requests/{id}/status`, авторизация `Authorization: Key ID:SECRET`, модель `higgsfield-ai/soul/standard`). Страница `/visualizaciya/[id]` после оплаты сабмитит генерацию по `buildPrompt(config)`, инлайн-поллинг ~12 c, состояние «рендерим…» с авто-обновлением; **фолбэк на курированный рендер** при любой ошибке/`not_enough_credits`/`failed` — клиент всегда получает картинку. request_id в `GenerationJob.inputUrl`, результат в `resultUrl` (без миграций). Включается `VISUALIZATION_PROVIDER=higgsfield`. **⚠️ БЛОКЕР: на API-аккаунте Higgsfield нет кредитов** (проверено curl — ключи валидны, ответ `403 not_enough_credits`). Чтобы живой режим реально генерировал: (а) пополнить кредиты на **cloud.higgsfield.ai**, (б) задать в Render `HIGGSFIELD_KEY_ID`/`HIGGSFIELD_KEY_SECRET` + `VISUALIZATION_PROVIDER=higgsfield`, (в) до этого работает curated. **Ключи присылались в чат — перевыпустить** (см. задачу «Ротация секретов»).
...
`<строка подключения убрана из репозитория — хранить только в .env / панели хостинга>`
