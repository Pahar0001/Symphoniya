...
Многостраничный сайт с каталогом и портфолио из БД, админ-панелью (включая редактор самого сайта)
и приёмом оплаты ЮKassa.

## Стек
Next.js 14 (App Router, TypeScript) · Tailwind CSS · Prisma + PostgreSQL · NextAuth (credentials) ·
ЮKassa REST API · Docker.
...
- **ЮKassa**: предоплата/задаток за проект.
- **Админ-панель** `/admin`: дашборд, аналитика, CRUD каталога и портфолио, отзывы, заказы — без прямого
  доступа к БД. Раздел **«Сайт»** (`/admin/site`): тексты и фото первого экрана, салоны, телефоны, соцсети.
...
5. При старте контейнер сам применит миграции и seed: создаст таблицы, демо-каталог
   и админ-пользователя. Демо-товары добавляются только при первом запуске
